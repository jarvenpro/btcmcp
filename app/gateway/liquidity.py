...
from app.gateway.http import GatewayHttpClient
    ...
    async def build_context(
        ...
        *,
        ...
        reference_price = (
            ...
            or bybit_book.get("mid_price")
            or (okx_book or {}).get("mid_price")
        )

        binance_liq_task = self._sample_binance_liquidations(symbol, liquidation_sample_seconds)
        bybit_liq_task = self._sample_bybit_liquidations(symbol, liquidation_sample_seconds)
        binance_liq_result, bybit_liq_result = await asyncio.gather(
            binance_liq_task,
            bybit_liq_task,
            return_exceptions=True,
        )

        binance_liquidations = binance_liq_result if isinstance(binance_liq_result, list) else []
        bybit_liquidations = bybit_liq_result if isinstance(bybit_liq_result, list) else []
        ...
        payload = {
            ...
            "venues": {
                "binance": {
                    ...
                    "liquidations": self._build_liquidation_summary("BINANCE", binance_liquidations, reference_price, liquidation_sample_seconds),
                },
                "bybit": {
                    "orderbook": self._build_orderbook_summary("BYBIT", bybit_book, reference_price),
                    "liquidations": self._build_liquidation_summary("BYBIT", bybit_liquidations, reference_price, liquidation_sample_seconds),
                },
            },
            "combined": {},
        }
